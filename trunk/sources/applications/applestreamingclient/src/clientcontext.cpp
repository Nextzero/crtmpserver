...
bool ClientContext::ConsumeAVBuffer() {
	...
	if ((pStream->GetCapabilities()->GetVideoCodecType() != CODEC_VIDEO_AVC)
	...
	if (_firstFeedTime == 0) {
		...
		_firstFeedTime = pStream->GetSpawnTimestamp();
	}

	//6. Feed
	while ((wallClockDelta + 1000 > (pStream->GetSpawnTimestamp() - _firstFeedTime)) &&
